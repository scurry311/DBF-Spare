...
当前最新阶段基线为 `v1.17.0-integrated-2x2-smoke`（2026-07-30）。馈电网络、
共地双层载板、无交叉扇出、探针和四个真实贴片已经完成一体化 HFSS 求解；
数值收敛且 EEP 功率误差仅 `0.86%`，但 passive/active/total RL 分别只有
`8.251/1.601/8.215 dB`，一体化 S4 与原 S8+S4 级联相差 `0.307`。因此失败根因
位于 POST 扇出和探针过渡，独立复验、4x4/16x16、标签和 critic 继续锁定。
