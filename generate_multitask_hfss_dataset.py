...
def write_hfss_export_vbs(path: Path, *, out_dir: Path) -> None:
    project_path = ROOT / "ura16_quick_10ghz_fullarray_run.aedt"
...
```powershell
python "{ps_path(ROOT / "generate_multitask_hfss_dataset.py")}" --samples-per-combo 10
```
...
```text
{ROOT / "ura16_quick_10ghz_fullarray_run.aedt"}
```
