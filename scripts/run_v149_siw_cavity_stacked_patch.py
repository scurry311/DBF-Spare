...
def validate_config(config: dict[str, Any]) -> None:
    ...
    if config.get("frequencies_ghz") != FROZEN_FREQUENCIES_GHZ:
        raise ValueError("v1.49 three-frequency grid is immutable")
    if config.get("sweep_ghz") != FROZEN_SWEEP_GHZ:
        raise ValueError("v1.49 frequency sweep is immutable")
    if config.get("scan") != FROZEN_SCAN:
        raise ValueError("v1.49 theta/phi scan grid is immutable")
    if config.get("resources") != FROZEN_RESOURCES:
        raise ValueError("v1.49 resource limits are immutable")
    if config.get("gates") != FROZEN_GATES:
        raise ValueError("v1.49 engineering gates are immutable")
    for forbidden in (
        "allow_2x2",
        "allow_4x4",
        "allow_16x16",
...
def aedt_execution_lock(lock_path: Path | None = None):
    path = lock_path or (ROOT / ".v149_aedt_execution.lock")
    try:
        descriptor = os.open(
        ...
        raise RuntimeError(
            f"Another v1.49 AEDT orchestration holds {path}"
        ) from error
    try:
        payload = (
            f"pid={os.getpid()}\n"
